...
-define(CALL_TAB, aect_test_utils_call_cache).
...
-record(call_id, {vsn, code, fun_name, args, backend}).
-record(call_cache_entry, {call_id:: #call_id{}, result:: term()}).
-record(decode_call_id, {code, fun_name, res, val}).
...
compile(Vsn, File) ->
    ...
    case ets:lookup(?COMPILE_TAB, CompilationId) of
        [#compilation_cache_entry{result = Result}] ->
            %% This should save at least 200ms per invocation
...
encode_call_data(Vsn, Code, Fun, Args) ->
    %% Lookup the res in the cache - if not present just calculate the result
    Backend = backend(),
    CallId = #call_id{vsn = Vsn, code = Code, fun_name = Fun, args = Args, backend = Backend},
    case ets:lookup(?CALL_TAB, CallId) of
        [#call_cache_entry{result = Result}] ->
            %% This should save at least 100ms per invocation
            ct:log("Encode call cache HIT  :)"),
            Result;
        [] ->
            ct:log("Encode call cache MISS :("),
            Result = encode_call_data_(Vsn, Code, Fun, Args, Backend),
            ets:insert_new(?CALL_TAB, #call_cache_entry{call_id = CallId, result = Result}),
            Result
    end.
...
decode_call_result(Code, Fun, Res, Value) ->
    %% Lookup the res in the cache - if not present just calculate the result
    DecodeCallId = #decode_call_id{code = Code, fun_name = Fun, res = Res, val = Value},
...
setup_contract_cache() ->
    ...
    ok = try_setup_cache(?CALL_TAB, #call_cache_entry.call_id),
